...
namespace MockDbSet
{
    ...
    {
        private static readonly MethodInfo _createQueryMethod = typeof(InMemoryAsyncQueryProvider).GetDeclaredMethods().Single(m => m.IsGenericMethodDefinition && m.Name == "CreateQuery");
        private static readonly MethodInfo _executeMethod = typeof(InMemoryAsyncQueryProvider).GetDeclaredMethods().Single(m => m.IsGenericMethodDefinition && m.Name == "Execute");
